...
class Discovery:
  ...
  async def start_udp_server(self):
    ...
    udp_srv.run("0.0.0.0", int(self.config.network['discoveryPort']), loop=self.loop)

    self.server = DiscoveryUDPServer(server=udp_srv, loop=self.loop, config=self.config)
